...
try{
  ...
  const structuredFallback=await page.evaluate(()=>{
    ...
    const contract=window.gdCreativeContractFallback();creativeContract=contract;const plan=window.gdNarrativeFallbackPlan(300),validation=window.gdValidateCreativeContract(contract),prompt=narrativePlanPrompt(300,'');
    ...
    return {contract,plan,validation,prompt};
  });
  ok('a numbered evergreen guide keeps its selection math and receives a structured-list arc',structuredFallback.validation.valid&&structuredFallback.contract.format.type==='presenter'&&/beş seçeneği aynı karar ölçütleriyle/i.test(structuredFallback.contract.promise.statement)&&/hangi koşulda.*hangi sınırda bozulur/i.test(structuredFallback.contract.storyEngine.drivingQuestion)&&/beş seçeneği ayrı kullanım durumu ve sınırıyla/i.test(structuredFallback.contract.storyEngine.structure)&&structuredFallback.plan.narrativeMode==='structured_list'&&structuredFallback.plan.chapters.length===8&&structuredFallback.plan.chapters.filter(ch=>/^Seçenek \d$/.test(ch.title)).length===5&&structuredFallback.plan.chapters.at(-1).title==='Koşula göre seçim haritası'&&/exactly 5 distinct items.*one separate chapter for every item/i.test(structuredFallback.prompt),structuredFallback);

  const fictionalMystery=await page.evaluate(()=>{
    const previous={topic,inputLang,fmt,creatorDNA:JSON.parse(JSON.stringify(creatorDNA||null)),projectGuidance:JSON.parse(JSON.stringify(projectGuidance||{})),creativeContract:JSON.parse(JSON.stringify(creativeContract||null))};
    topic='Tamamen kurmaca ve geriye dönük bir hikâye: Koridordaki ampulün neden açık kaldığını üç somut ipucuyla araştırıyoruz.';inputLang='tr';fmt='essay';creatorDNA={v:1,outcome:'feel',carrier:'story',presence:'voice',pace:'reflective'};projectGuidance={outcome:'feel',approach:'observe',production:'voice-footage',stage:'pre-shoot',storyReality:'fictional',narratorTime:'retrospective',context:'Final ampulün neden açık kaldığını açıkça çözmeli. İpuçları taşınma bandı, kutu izi ve anahtardaki not.'};
    const contract=window.gdCreativeContractFallback();creativeContract=contract;const plan=window.gdNarrativeFallbackPlan(360),validation=window.gdValidateCreativeContract(contract);
    topic=previous.topic;inputLang=previous.inputLang;fmt=previous.fmt;creatorDNA=previous.creatorDNA;projectGuidance=previous.projectGuidance;creativeContract=previous.creativeContract;return {contract,plan,validation};
  });
  ok('a fictional mystery locks its exact why-question, named-clue chain and concrete resolution',fictionalMystery.validation.valid&&fictionalMystery.contract.storyEngine.storyReality==='fictional'&&fictionalMystery.contract.storyEngine.narratorTime==='retrospective'&&fictionalMystery.contract.storyEngine.drivingQuestion==='Ampul neden açık kaldı?'&&/üç somut ipucunun.*ilk açıklamayı.*gerçek nedenini/i.test(fictionalMystery.contract.promise.statement)&&/üç adlandırılmış ipucunu.*çelişkiyle.*somut nedeni/i.test(fictionalMystery.contract.storyEngine.structure)&&fictionalMystery.plan.narrativeMode==='investigation'&&fictionalMystery.plan.chapters.at(-2).title==='Yanıt'&&fictionalMystery.plan.chapters.at(-1).title==='Son görüntü',fictionalMystery);

  const visualEssay=await page.evaluate(()=>{
    const previous={topic,inputLang,fmt,creatorDNA:JSON.parse(JSON.stringify(creatorDNA||null)),projectGuidance:JSON.parse(JSON.stringify(projectGuidance||{})),creativeContract:JSON.parse(JSON.stringify(creativeContract||null))};
    topic='Bir şehrin sessiz denilen saatini araştıran 7 dakikalık görsel essay: gece dört sokak köşesinde sabit ses ve görüntü kaydı alacağız.';inputLang='tr';fmt='essay';creatorDNA={v:1,outcome:'understand',carrier:'story',presence:'voice',pace:'reflective'};projectGuidance={outcome:'understand',approach:'observe',production:'voice-footage',stage:'pre-shoot',context:'Şehrin sessiz olduğu varsayımını, dört somut gözlem ve onu zorlayan karşı örnekle incelemek istiyorum. Sonuç henüz bilinmiyor.'};
    const contract=window.gdCreativeContractFallback();creativeContract=contract;const plan=window.gdNarrativeFallbackPlan(420),validation=window.gdValidateCreativeContract(contract);
    topic=previous.topic;inputLang=previous.inputLang;fmt=previous.fmt;creatorDNA=previous.creatorDNA;projectGuidance=previous.projectGuidance;creativeContract=previous.creativeContract;return {contract,plan,validation};
  });
  ok('an explicit visual essay overrides story-led DNA with an argument, counterexample and bounded synthesis',visualEssay.validation.valid&&visualEssay.contract.storyEngine.productionState==='pre_shoot'&&visualEssay.contract.storyEngine.narratorTime==='prospective'&&/başlangıç varsayımı.*somut gözlem ve karşı örneklerle nasıl değişecek/i.test(visualEssay.contract.storyEngine.drivingQuestion)&&/somut bir gözlemle aç.*karşı örnekle.*daha dar bir senteze var.*son görüntüde bitir/i.test(visualEssay.contract.storyEngine.structure)&&visualEssay.plan.narrativeMode==='essay'&&visualEssay.plan.chapters.some(ch=>ch.title==='Karşı görüş')&&visualEssay.plan.chapters.at(-1).title==='Son görüntünün taşıdığı fikir',visualEssay);

  const factualDocumentary=await page.evaluate(()=>{
    const previous={topic,inputLang,fmt,creatorDNA:JSON.parse(JSON.stringify(creatorDNA||null)),projectGuidance:JSON.parse(JSON.stringify(projectGuidance||{})),creativeContract:JSON.parse(JSON.stringify(creativeContract||null))};
    topic='Mahalledeki ayakkabı tamircisinin bir iş gününü, tek bir çift çizmenin onarımı üzerinden anlatan 8 dakikalık kısa belgesel.';inputLang='tr';fmt='essay';creatorDNA={v:1,outcome:'feel',carrier:'story',presence:'voice',pace:'reflective'};projectGuidance={outcome:'feel',approach:'observe',production:'voice-footage',stage:'pre-shoot',context:'Çekim yarın yapılacak. Ustanın dükkânı bina tadilatı nedeniyle ay sonunda kapanacak; onarımın sonucu ve gün içindeki değişim henüz bilinmiyor.'};
    const contract=window.gdCreativeContractFallback();creativeContract=contract;const plan=window.gdNarrativeFallbackPlan(480),validation=window.gdValidateCreativeContract(contract);
    topic=previous.topic;inputLang=previous.inputLang;fmt=previous.fmt;creatorDNA=previous.creatorDNA;projectGuidance=previous.projectGuidance;creativeContract=previous.creativeContract;return {contract,plan,validation};
  });
  ok('an explicit factual documentary keeps the subject, verified context, pressure and unfilmed change separate',factualDocumentary.validation.valid&&factualDocumentary.contract.storyEngine.productionState==='pre_shoot'&&factualDocumentary.contract.storyEngine.storyReality==='factual'&&factualDocumentary.contract.storyEngine.narratorTime==='prospective'&&/öznenin gündelik işi.*hangi somut anlarda görünür kılacak/i.test(factualDocumentary.contract.storyEngine.drivingQuestion)&&/özneyle sürecin ortasında tanış.*doğrulanmış bağlam.*baskıyı eylem üzerinde göster.*son görüntüde bitir/i.test(factualDocumentary.contract.storyEngine.structure)&&factualDocumentary.plan.narrativeMode==='documentary'&&factualDocumentary.plan.chapters.some(ch=>ch.title==='Baskı')&&factualDocumentary.plan.chapters.at(-1).title==='Son görüntünün bıraktığı yer',factualDocumentary);

  const experientialStory=await page.evaluate(()=>{
    const previous={topic,inputLang,fmt,creatorDNA:JSON.parse(JSON.stringify(creatorDNA||null)),projectGuidance:JSON.parse(JSON.stringify(projectGuidance||{})),creativeContract:JSON.parse(JSON.stringify(creativeContract||null))};
    topic='On dört gün boyunca her gün yalnızca tek kare çektiğim geriye dönük bir görsel hikâye.';inputLang='tr';fmt='essay';creatorDNA={v:1,outcome:'feel',carrier:'story',presence:'voice',pace:'reflective'};projectGuidance={outcome:'feel',approach:'personal',production:'voice-footage',stage:'post-shoot',storyReality:'dramatized',narratorTime:'retrospective',context:'Telefonla çok fazla fotoğraf çekip hiçbirini seçememek bu deneyi başlattı. Merkez soru: Günde tek kare çekmek neyi fotoğrafladığımı ve karar verme biçimimi nasıl değiştirdi? Finalde on dört baskıyı masaya dizip kamerayı kapatıyorum.'};
    const contract=window.gdCreativeContractFallback();creativeContract=contract;const plan=window.gdNarrativeFallbackPlan(420),validation=window.gdValidateCreativeContract(contract);
    topic=previous.topic;inputLang=previous.inputLang;fmt=previous.fmt;creatorDNA=previous.creatorDNA;projectGuidance=previous.projectGuidance;creativeContract=previous.creativeContract;return {contract,plan,validation};
  });
  ok('an experiential story preserves supplied motivation, exact question, resistance, earned result and final action',experientialStory.validation.valid&&experientialStory.contract.storyEngine.storyReality==='dramatized'&&experientialStory.contract.storyEngine.narratorTime==='retrospective'&&experientialStory.contract.storyEngine.drivingQuestion==='Günde tek kare çekmek neyi fotoğrafladığımı ve karar verme biçimimi nasıl değiştirdi?'&&/neden başlandığını.*deneme ve direnç.*somut sonucun kazanıldığını/i.test(experientialStory.contract.promise.statement)&&/somut motivasyonla aç.*bağlayıcı kararı göster.*direnci görünür kıl.*somut sonuçla cevapla.*son fiziksel eylemde bitir/i.test(experientialStory.contract.storyEngine.structure)&&experientialStory.plan.narrativeMode==='experiential_process'&&experientialStory.plan.chapters.some(ch=>ch.title==='Zorluk')&&experientialStory.plan.chapters.at(-1).title==='Son eylem',experientialStory);
