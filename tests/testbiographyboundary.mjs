...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    currentNarrativePlan=window.gdNarrativeFallbackPlan(60);
  ...
  ok('a failed contextual patch quarantines only unsafe lines and schedules their chapters instead of discarding the draft',result.fallbackGate.repairMode==='local_quarantine'&&result.fallbackGate.audit.status==='clear'&&result.fallbackGate.forcedRepairChapterIds.length===2&&result.history.some(x=>x.status==='rejected'&&x.origin==='epistemic_rewrite_failed')&&result.history.some(x=>x.status==='promoted'&&x.origin==='epistemic_local_quarantine'),{fallback:result.fallbackGate,history:result.history});
