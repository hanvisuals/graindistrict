...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const editorialPlan=JSON.parse(JSON.stringify(postShootPlan));editorialPlan.storyBible.chronologyMode='linear';editorialPlan.storyBible.endingAction='The notebook closes and the phone stays on the table.';
  ...
  ok('the whole-script review detects repeated event openings, fixed-fact conflicts and bare date fragments',result.continuityIssues.some(issue=>issue.code==='VOICEOVER_EVENT_OVERLAP')&&result.continuityIssues.some(issue=>issue.code==='VOICEOVER_FIXED_FACT_CONTRADICTION')&&result.continuityIssues.some(issue=>issue.code==='VOICEOVER_TEMPORAL_FRAGMENT'),result.continuityIssues);
