...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const safeIssues=window.gdPreShootTruthIssues(safePreShoot,fallback),safePresentIssues=window.gdPreShootTruthIssues(safePresent,fallback);
    ...
    apiRetry=()=>Promise.resolve(JSON.stringify({script:presentOutcome,notes:['unsafe observed results returned by simulated editor']}));epistemicGateCandidateScript=candidate=>Promise.resolve({script:candidate,audit:null,rewritten:false});
    ...
    return {fallback,validation,repeatedValidation,flatValidation,chronologyValidation,context,prompt,generationPrompt,modes,headers,quality,duplicate,uncertainty,fabricatedIssues,fabricatedQuality,presentOutcomeIssues,presentOutcomeQuality,presentOutcomeBlocked,safeIssues,safePresentIssues,localFallback,fallbackVoice,editorialTruthResult,dramatizedIssues,dramatizedAudit,dramatizedContext,leakIssues,leakBlocked,replacementReport,spliced,splicedQuality,editorialIssues,continuityIssues,completeEditorial,truncatedEditorial,autoResult,forcedResult,advisoryResult,savedPlan,restored:currentNarrativePlan,direction:Array.from(document.querySelectorAll('#scriptDirection span')).map(x=>x.textContent)};
  });
