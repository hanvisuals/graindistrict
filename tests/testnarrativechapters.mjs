...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    return {fallback,validation,repeatedValidation,flatValidation,chronologyValidation,context,prompt,generationPrompt,modes,headers,quality,duplicate,uncertainty,fabricatedIssues,fabricatedQuality,safeIssues,localFallback,dramatizedIssues,dramatizedAudit,dramatizedContext,leakIssues,leakBlocked,replacementReport,spliced,splicedQuality,editorialIssues,continuityIssues,completeEditorial,truncatedEditorial,autoResult,forcedResult,advisoryResult,savedPlan,restored:currentNarrativePlan,direction:Array.from(document.querySelectorAll('#scriptDirection span')).map(x=>x.textContent)};
  });
