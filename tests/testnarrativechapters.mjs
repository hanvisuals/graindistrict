...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    return {fallback,validation,repeatedValidation,flatValidation,context,prompt,generationPrompt,modes,headers,quality,duplicate,uncertainty,fabricatedIssues,fabricatedQuality,safeIssues,localFallback,replacementReport,spliced,splicedQuality,autoResult,forcedResult,advisoryResult,savedPlan,restored:currentNarrativePlan,direction:Array.from(document.querySelectorAll('#scriptDirection span')).map(x=>x.textContent)};
  });
