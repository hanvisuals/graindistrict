...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    return {fallback,validation,repeatedValidation,flatValidation,chronologyValidation,context,prompt,generationPrompt,modes,headers,quality,duplicate,uncertainty,fabricatedIssues,fabricatedQuality,safeIssues,localFallback,dramatizedIssues,dramatizedAudit,dramatizedContext,leakIssues,leakBlocked,replacementReport,spliced,splicedQuality,editorialIssues,completeEditorial,truncatedEditorial,autoResult,forcedResult,advisoryResult,savedPlan,restored:currentNarrativePlan,direction:Array.from(document.querySelectorAll('#scriptDirection span')).map(x=>x.textContent)};
  });
  ...
  ok('generation runs a whole-script editorial pass after chapter repair',/editorialPolishNarrativeScript\(result\.script,currentNarrativePlan/.test(source)&&/Editing the whole story/.test(source)&&/script_revision/.test(source),null);
